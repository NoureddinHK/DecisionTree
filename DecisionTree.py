import pandas as pd  # برای کار با دیتافریم
import numpy as np  # برای محاسبات ریاضی و آرایه ها
from sklearn.datasets import make_classification  # تولید داده مصنوعی
from sklearn.preprocessing import StandardScaler, KBinsDiscretizer  # برای استانداردسازی و گسسته‌سازی داده‌ها
from sklearn.model_selection import train_test_split  # برای تقسیم داده‌ها به آموزش و آزمایش
import os  # برای ذخیره فایل
import warnings  # warning handling
import pydot  # making graphviz graphs


warnings.filterwarnings("ignore")  # to not print warnings in the output

class NewsPopularityProcessor:  # main class
    """Class to process news popularity data, calculate feature metrics, and build/visualize a decision tree."""
    
    class Node:  # decision tree node class
        ...
        def __init__(self, feature_idx=None, split_value=None, left=None, right=None, label=None, gini=None, info_gain=None, n_samples=None):  # سازنده کلاس گره با پارامترهای پیش‌فرض
            ...
            self.split_value = split_value  # value threshold for splitting
            self.left = left  # فرزند چپ گره
            self.right = right  # فرزند راست گره
            self.label = label  # این کلاس نود هایی است که گره نیستند بلکع بلکه برگ هستند
            self.gini = gini  # Gini index for the node
            self.info_gain = info_gain  # information gain for the split (None for leaf nodes)
            self.n_samples = n_samples  # number of samples in the current node
    ...
    def entropy(y):
        ...
        if not np.all(np.isclose(y, np.round(y))):  # بررسی اینکه آیا تمام مقادیر y اعداد صحیح هستند
            raise ValueError(f"Error in entropy: y contains non-integer values: {np.unique(y)}")  #  خطا در صورت وجود مقادیر غیرصحیح
    ...
    @staticmethod
    def information_gain(X, y, feature_idx):  # تعریف متد استاتیک برای محاسبه info-gain
    ...
    def __init__(self, dataset_path='OnlineNewsPopularity.csv', n_samples=10000, n_features=20, n_informative=15, n_redundant=5, random_state=42, max_depth=5, min_samples_split=2):  # سازنده کلاس اصلی
        ...
        self.discretizer = None  # شی گسسته‌سازی
    ...
    def load_data(self):  # متد برای بارگذاری و پیش‌پردازش دیتاست
        ...
        try:  # error handling block
            ...
            raise FileNotFoundError(f"File '{self.dataset_path}' not found. Please ensure the file is uploaded in the correct directory.")  # خطا اگر فایل در محل درست نبود
    ...
    def generate_synthetic_data(self):  # متد برای تولید داده‌های مصنوعی
        ...
        self.X_new, self.y_new = make_classification(  # تولید داده‌های مصنوعی با استفاده از make_classification
            ...
            random_state=self.random_state,  # استیت تصادفی برای تکرارپذیری
    ...
    def standardize_and_discretize(self):  # متد برای استانداردسازی و گسسته‌سازی ویژگی‌ها
        ...
        scaler = StandardScaler()  # ایجاد شی استانداردساز
...
############### خسته شودم
    ...
    def prune_tree(self, node):  # پس هرس
        ...
        if node.left.label is not None and node.right.label is not None and node.left.label == node.right.label:  # if both children are leaves with the same label
    ...
    def predict_single(self, x, node):  # متد برای پیش‌بینی یک نمونه
        ...
        if node.label is not None:  # اگر گره برگ است
            return node.label  # return class label
    ...
    def visualize_tree(self):  # متد برای نمایش متنی درخت تصمیم
        ...
        def print_node(node, depth=0, prefix="", is_last=True):  # تابع داخلی برای چاپ بازگشتی گره‌ها
            ...
            indent = "|  " * (depth - 1) + ("|-- " if depth > 0 else "")  # the characters to indent the tree structure
    ...
    def visualize_tree_graphviz(self):  # متد برای نمایش گرافیکی درخت با گرافویز
        ...
        def add_node_to_graph(graph, node, node_id, parent_id=None, edge_label=None):  # تابع داخلی برای اضافه کردن گره‌ها به گراف
            ...
            if node.label is not None:  # if node is leaf
                # Leaf node
                label = f"Class: {node.label}\\nGini: {node.gini:.4f}\\nSamples: {node.n_samples}"  # برچسب گره برگ
                node_color = "lightgreen" if node.label == 0 else "lightcoral"  # lightgreen for class 0, lightcoral for class 1 (class 0 means not popular, class 1 means popular)
            else:  # اگر گره تصمیم است
                # Decision node
                feature_name = self.feature_columns[node.feature_idx]  # نام ویژگی
            ...
            if parent_id is not None:  # if parent node exists
                edge = pydot.Edge(parent_id, node_id, label=edge_label)  # add edge or label
                graph.add_edge(edge)  # add edge to the graph
    ...
    def run(self):  # متد ران کردن داستان
